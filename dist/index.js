...
__export(index_exports, {
  ...
  FileExternalizer: () => FileExternalizer,
  GeminiAdapter: () => GeminiAdapter,
  OllamaAdapter: () => OllamaAdapter,
  OpenAIAdapter: () => OpenAIAdapter,
  Telemetry: () => Telemetry,
  TextOptimizer: () => TextOptimizer,
  ToolConsolidator: () => ToolConsolidator,
  addTrustedDomain: () => addTrustedDomain,
  auditDirectory: () => auditDirectory,
  auditSkill: () => auditSkill,
  auditSkillContent: () => auditSkillContent,
...
var FileExternalizer = class {
   ...
   */
  async init() {
    if (!(0, import_fs.existsSync)(this.tempDir)) {
      await (0, import_promises.mkdir)(this.tempDir, { recursive: true });
    }
  }
  /**
  ...
  async externalize(params) {
    ...
    const fileId = (0, import_crypto.randomUUID)().slice(0, 8);
    const fileName = `${toolName}_${fileId}.json`;
    const filePath = (0, import_path.join)(this.tempDir, fileName);
    await (0, import_promises.writeFile)(filePath, resultString, "utf8");
  ...
  async cleanup() {
    ...
    try {
      const files = await (0, import_promises.readdir)(this.tempDir);
      for (const file of files) {
        const filePath = (0, import_path.join)(this.tempDir, file);
