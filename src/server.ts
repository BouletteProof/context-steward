...
const TOOLS = [
  ...
  { name: 'import_scores', description: 'Import external telemetry scores (e.g. from BPOS COCA pipeline) into the outcome store and apply learnings to skill files. Accepts an array of {slug, score, notes, intent?} objects. Scores >= 0.8 generate [STRENGTH] entries, scores < 0.5 generate [WEAKNESS] entries. Use this to bridge server-side telemetry into local skill improvement.', inputSchema: { type: 'object' as const, properties: { scores: { type: 'array', items: { type: 'object', properties: { slug: { type: 'string' }, score: { type: 'number' }, notes: { type: 'string' }, intent: { type: 'string' } }, required: ['slug', 'score', 'notes'] } }, source: { type: 'string', description: 'Where these scores come from, e.g. bpos-coca' } }, required: ['scores'] } },
];
